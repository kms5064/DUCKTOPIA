...
const coreDamaged = ({ socket, payload }) => {
  ...
  game.broadcast(makePacket(config.packetType.S_OBJECT_HP_UPDATE_NOTIFICATION, { coreHp: hp }));

  if (hp <= 0) {
    game.gameOver();
    game.broadcast(makePacket(config.packetType.S_GAME_OVER_NOTIFICATION));
  }
};
